...
import { Crest, LiveDot, BottomNav, Empty } from '@/app/ui'
      ...
      {tabs.length > 0 && (
        <div className="flex gap-1 px-3.5 pb-3 overflow-x-auto border-b border-chalk/[0.06]">
          {tabs.map(t => {
            ...
            return (
              <Link key={t.key} href={`/matches?date=${t.key}`}
...
function MatchRow({ m, first }: { m: any; first: boolean }) {
  ...
  return (
    <Link href={`/match/${m.match_id}`}
      className={`grid items-center gap-2 px-3 py-3 active:bg-[#1C1C22]
        [grid-template-columns:1fr_60px_1fr]
      ...
      <div className="flex flex-col items-center justify-center">
        ...
        {live
          ...
          : m.field
          ? <span className="text-[8px] text-off mt-1 truncate max-w-[58px]">{m.field}</span>
          : null}
      </div>
