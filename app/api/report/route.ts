...
ΥΦΟΣ
Ζωντανό, περιγραφικό αθλητικό ρεπορτάζ — σαν να έζησε ο αναγνώστης τον αγώνα. Πλούσια αφήγηση με ροή: πώς ξεκίνησε, πώς εξελίχθηκε, πού κρίθηκε.
Πολύ χιούμορ και καλαμπούρι, πειραχτικό αλλά ΠΟΤΕ προσβλητικό. Οι ατάκες και οι παρομοιώσεις βγαίνουν από τα ίδια τα γεγονότα του αγώνα.
Δώσε χαρακτήρα στις στιγμές: ρυθμός, ένταση, ατμόσφαιρα, ανατροπές, καθοριστικές φάσεις. Κάνε τους παίκτες πρωταγωνιστές — περίγραψε τι σήμανε το κάθε γκολ για το ματς.
...
ΚΑΝΟΝΕΣ
- Γράψε ΜΟΝΟ την περιγραφή, 3-5 παραγράφους, με πλούσιο αλλά ρέον λόγο (όχι φλύαρο, όχι κλισέ).
- Περίγραψε ΚΑΘΕ γκολ ξεχωριστά, με το λεπτό του και τι σήμανε για το σκορ, δένοντάς τα σε αφήγηση.
  Παράδειγμα ύφους: «ο Γιάννης πέτυχε το πρώτο γκολ της ομάδας του στο 12', και στο 20' ο Νίκος διπλασίασε με ένα όμορφο σουτ». Αν ένα γκολ είχε ασίστ, ανάφερε ποιος έδωσε την πάσα.
- Humanize: γράψε σαν άνθρωπος-ρεπόρτερ, ζωντανά και φυσικά — όχι σαν λίστα ή ρομπότ. Χιούμορ ΝΑΙ, αλλά πάντα να ΠΕΡΙΓΡΑΦΕΙ την ουσία της φάσης.
- Χρησιμοποίησε ΑΠΟΚΛΕΙΣΤΙΚΑ τα γεγονότα που σου δίνονται. ΜΗΝ εφευρίσκεις γκολ, παίκτες, λεπτά, φάσεις, σουτ, δοκάρια ή στατιστικά που δεν υπάρχουν.
- Χτίσε αφήγηση: άνοιγμα με την εικόνα του αγώνα, μετά η εξέλιξη ανά ημίχρονο, και κλείσιμο με το πώς κρίθηκε.
...
${timeline || '(δεν καταγράφηκαν φάσεις)'}`
    ...
    if (process.env.ANTHROPIC_API_KEY) {
      ...
      const msg = await client.messages.create({
        // Πιο προηγμένο μοντέλο για πλουσιότερο, πιο «ανθρώπινο» ρεπορτάζ.
        // Το Opus 5 τρέχει adaptive thinking από default — δίνουμε μεγαλύτερο
        // max_tokens ώστε να χωράει η σκέψη + το κείμενο (χρεώνονται μόνο όσα παραχθούν).
        model: 'claude-opus-5',
        max_tokens: 6000,
        system: SYSTEM,
