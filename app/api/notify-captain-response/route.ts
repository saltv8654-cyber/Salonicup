...
export async function POST(req: Request) {
  ...
  const { data: m } = await admin.from('matches')
    .select('match_date, team_a_data:team_a(name), team_b_data:team_b(name), league:league_id(name)')
  ...
  const payload = JSON.stringify({
    title: status === 'postpone' ? '⛔ Αίτημα αναβολής' : '🕐 Αίτημα αλλαγής ώρας',
    body: `${who} ${LBL[status]}: ${teamName}${note ? ` — «${note}»` : ''}`,
    url: '/admin/matches',
  })
