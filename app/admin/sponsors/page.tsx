...
type Sponsor = { id: string; name: string; logo_url: string | null; tier: string; sort: number }
...
export default function AdminSponsors() {
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto flex flex-col gap-4">
      ...
      {open && <SponsorForm onClose={() => setOpen(false)} onSaved={() => { setOpen(false); fetchRows() }} />}
    </div>
  )
}

function SponsorForm({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const supabase = createClient()
  ...
  async function save() {
    ...
    setBusy(true)
    const { error } = await supabase.from('sponsors').insert({ name: name.trim(), logo_url: logo || null, tier })
    setBusy(false)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/70 flex items-end sm:items-center justify-center p-4"
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-[15px] font-extrabold text-chalk">Νέος χορηγός</h2>
